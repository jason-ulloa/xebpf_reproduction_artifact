...
USER_NAME="${1:-xebpfexp}"; REPS="${2:-3}"; DURATION="${3:-120}"; RATE="${4:-4}"; KEY_PATH="${5:-}"; RESULT_ROOT="${6:-/var/lib/ebpf-research}"
...
python3 "$SCRIPT_DIR/run_helpers.py" preflight --user "$USER_NAME" --workload "$SCRIPT_DIR/attack_emulation_workloads.py"
mkdir -p "$RESULT_ROOT"; chmod 700 "$RESULT_ROOT"
python3 "$SCRIPT_DIR/production_safety_gate.py" --user "$USER_NAME" --duration "$DURATION" --rate "$RATE" --hmac-key-path "$KEY_PATH" --report "$RESULT_ROOT/ATTACK_EMULATION1_SAFETY_GATE.json"
...
mkdir -p "$RESULT_ROOT"; chmod 700 "$RESULT_ROOT"
python3 - "$RESULT_ROOT/ATTACK_EMULATION1_PROTOCOL.json" "$REPS" "$DURATION" "$RATE" <<'PY2'
...
PY2
chmod 600 "$RESULT_ROOT/ATTACK_EMULATION1_PROTOCOL.json"
CPID=""
...
for idx in "${!PROFILES[@]}"; do
 ...
 for i in $(seq 1 "$REPS"); do
  ...
  python3 "$SCRIPT_DIR/collector.py" --experiment-id "$EXP" --workload-id "$PROFILE" --label malicious --attack-id "$ATTACK" --target-uid "$UID_NUM" --duration "$((DURATION+8))" --hmac-key-path "$KEY_PATH" --output-dir "$RESULT_ROOT" & CPID=$!
  sleep 4
  ...
  wait "$CPID"; CPID=""
  python3 "$SCRIPT_DIR/run_helpers.py" mark --root "$RESULT_ROOT" --experiment-id "$EXP" --workload-id "$PROFILE" --duration "$DURATION" --rate "$RATE" --user "$USER_NAME"
  sleep 2
 done
done
trap - EXIT INT TERM
