...
python3 "$SCRIPT_DIR/run_helpers.py" preflight --user "$USER_NAME" --workload "$SCRIPT_DIR/benign_workloads.py"
...
python3 - "$RESULT_ROOT/BENIGN1_PROTOCOL.json" "$REPS" "$DURATION" "$RATE" <<'PY2'
...
PY2
chmod 600 "$RESULT_ROOT/BENIGN1_PROTOCOL.json"
CPID=""
...
for PROFILE in "${PROFILES[@]}"; do
 ...
 for i in $(seq 1 "$REPS"); do
  ...
  python3 "$SCRIPT_DIR/collector.py" --experiment-id "$EXP" --workload-id "$PROFILE" --label benign --attack-id none --target-uid "$UID_NUM" --duration "$((DURATION+8))" --hmac-key-path "$KEY_PATH" --output-dir "$RESULT_ROOT" & CPID=$!
  sleep 4
